...
"""
function bloch_plot!(gp::GridPosition, times::Vector{<:Real}, states::Vector{<:StateVector})
    lscene = LScene(gp, 
        show_axis = false,
        # tellwidth = false,
        # tellheight = false
...
function population_plot!(gp::GridPosition, times::Vector{<:Real}, states::Vector{<:StateVector})
    ax = Axis(gp, xlabel = "Time", ylabel = "Population", title = "Population over time")
...
function alignment_plot!(gp::GridPosition, times::Vector{<:Real}, states::Vector{<:StateVector}, HamiltonianTD::Function)
    ax = Axis(gp, xlabel = "Time", ylabel = "Alignment", title = L"Alignment with $\hat{H}$ Eigenvector")
    hamiltonian_eigenkets = getindex.(eigenstates.(dense.(HamiltonianTD.(times))), 2)
    bra_states = dagger.(states)
    for i in (1:length(hamiltonian_eigenkets[1]))
